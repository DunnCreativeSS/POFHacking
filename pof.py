...
for user in users:
    try:
        ...
        if(sentiment_analyzer_scores(user.description.encode("utf-8"))['compound'] > 0.985):
            if user.online == True:
...
#broadcastMessage(tomsg, "Hey! This is going to sound terrrrribly awkward but I hacked POF in a very special way that allows me to rank different profiles among the online users to see whose description matches an analysis I'd created that might indicate we'd have a swell chance at a good conversation - so if this isn't terribly scary as well as terribly awkward, do hazard a response back and we'll see if I'm right?")
